...
int fota_app_on_install_candidate(const char *candidate_fs_name, const manifest_firmware_info_t *firmware_info)
{
    ...
    int length = snprintf(command,
                          ...
                          "%s %s %s",
                          "/mcce/fota_update_activate.sh",  candidate_fs_name, MBED_CLOUD_CLIENT_FOTA_LINUX_HEADER_FILENAME);
...
int fota_app_on_main_app_verify_install(const fota_header_info_t *expected_header_info)
{
    ...
    for(int x=0; x < FOTA_MANIFEST_VENDOR_DATA_SIZE; x++)
    {
        printf("%c",expected_header_info->vendor_data[x]);
    }
    printf("\n");
    printf("DYER: vendor_data: %s\n",expected_header_info->vendor_data);
